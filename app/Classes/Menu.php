            ...
            foreach ($items as $item) {
                if (isset($item['submenu']) && $item['submenu']) {
                    ...
                    foreach ($item['submenu'] as $submenu) {
                        $content .= '<li><a class="dropdown-item' . (request()->is(str_replace('.', '/', $submenu['page-group']) . "*")) . '"' . (isset($submenu['new-tab']) && $submenu['new-tab'] ? 'target="_blank"' : '') . ' href=' . route($submenu['page']) . '>' . $submenu['title'] . '</a></li>';
                    }
                    $content .= '</ul></li>';
                } else {
                    ...
                    // $content .= '<li class="nav-item"><a class="nav-link"' . (isset($item['new-tab']) && $item['new-tab'] ? 'target="_blank"' : '') . ' href=' . route($item['page']) . '>' . $item['title'] . '</a></li>';
                }
            }
        }
        echo $content;
