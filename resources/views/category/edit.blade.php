    ...
    <div class="row justify-content-center">
        <div class="col-md-10">
            <div class="card">
                ...
                <div class="card-body">
                    <form action="{{ route($item->id ? 'admin.categories.update' : 'admin.categories.store', ['category'=> $item->id]) }}" method="POST" enctype="multipart/form-data">
                        {!! csrf_field() !!}
                        @if($item->id)
                ...
                <div class="card-footer">
                    <a href="{{ route('admin.categories.index') }}" class="btn btn-primary btn-sm">{{__('Back')}}</a>
                </div>
            </div>
        </div>
    </div>
