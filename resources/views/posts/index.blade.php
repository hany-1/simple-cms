...
<script>
    $(document).ready(function() {
        var table = $('#post-table').DataTable({
            ...
            columnDefs: [{
                ...
                render: function(data, type, full, meta) {
                    var editUrl = Helper.route("{{route('admin.posts.edit',':1')}}", [full.id]);
        ...
        $('#post-table').on('click', '.deleteBtn', function() {
            if (confirm("Delete post?")) {
                //using axios or jquery way to handle delete post function
                var id = $(this).data('id');
                var datatable = $('#post-table').DataTable();

                axios.delete(Helper.route("{{route('admin.posts.destroy', ':1')}}", [id])).then(response => {
